...
// Arama butonu ve inputları seç
const filterButton = document.querySelector("#filter-button button");
const ilSelect = document.getElementById("adres-il");
const ilceSelect = document.getElementById("adres-ilce");
...
const minYearInput = document.querySelector(".year-inputs input:first-of-type");
const maxYearInput = document.querySelector(".year-inputs input:last-of-type");
const minKmInput = document.querySelector(".km-inputs input:first-of-type");
const maxKmInput = document.querySelector(".km-inputs input:last-of-type");
...
filterButton.addEventListener("click", () => {
    ...
    const minKm = parseInt(minKmInput.value) || 0;
    const maxKm = parseInt(maxKmInput.value) || Infinity;

    rows.forEach(row => {
        const il = row.querySelector("td:nth-child(8)").textContent.split("/")[0].trim();
        const ilce = row.querySelector("td:nth-child(8)").textContent.split("/")[1]?.trim();
        const price = parseFloat(row.querySelector("td:nth-child(6)").textContent.replace(" TL", "")) || 0;
        const year = parseInt(row.querySelector("td:nth-child(3)").textContent) || 0;
        const km = parseInt(row.querySelector("td:nth-child(4)").textContent.replace(/\D/g, "")) || 0;

        // Filtreleme kriterleri
        const matchesIl = !selectedIl || il === selectedIl;
        const matchesIlce = !selectedIlce || ilce === selectedIlce;
        const matchesPrice = price >= minPrice && price <= maxPrice;
        const matchesYear = year >= minYear && year <= maxYear;
        const matchesKm = km >= minKm && km <= maxKm;

        // Satırı göster veya gizle
        if (matchesIl && matchesIlce && matchesPrice && matchesYear && matchesKm) {
